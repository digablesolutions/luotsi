...
The command returns a JSON envelope with a five-minute path, concrete commands, an AI-agent prompt, and the local-first/artifact-first boundary that explains where Luotsi fits beside automation frameworks and hosted device services. If you do not pass `--device`, the first command is `luotsi doctor` so Luotsi can list adb-visible devices and return the exact selected-device next command.
