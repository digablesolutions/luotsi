...
4. `artifacts.artifact_root` when no richer command field exists, then run `luotsi replay packet --artifacts <artifact-root>` first so the loop has `run-summary.json` and `run-summary.md`. Use `luotsi replay open --artifacts <artifact-root> --dry-run` when a human needs the replay front door response, and use `luotsi artifacts open <artifact-root>` only when you specifically need the generic artifact browser.
