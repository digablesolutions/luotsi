...
When the starting point is an Android CLI Journey-style intent (a Journey objective; not an Android `Intent` object),
capture that intent first with the repository's journey intake template, then promote reviewed
