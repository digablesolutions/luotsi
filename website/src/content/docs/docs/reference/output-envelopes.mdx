...
- `replay capsule` returns a failure-capsule summary with `primary_failure`, `failure_timeline`, `artifact_manifest`, optional scenario draft summary fields including bounded `next_actions`, draft-led `recommended_next_steps` when persisted draft actions exist, and `suggested_commands`.
