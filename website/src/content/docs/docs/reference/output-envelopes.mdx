...
- `replay capsule` returns a failure-capsule summary with `primary_failure`, `failure_timeline`, `artifact_manifest`, optional scenario draft summary fields including bounded `next_actions` and `run_handoff`, draft-led `recommended_next_steps` that promote ready dry-run/preflight/run commands first, and `suggested_commands`.
