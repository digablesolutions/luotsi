...
- `replay graph` returns a stable graph payload with `query`, `agent_summary`, counts, taxonomy, insights, actions, evidence, facts, causal chains, hypotheses, optional output paths, plus returned `nodes` and `edges`.
