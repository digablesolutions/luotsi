...
6. `data.commands[]`, `data.artifact_commands[]`, or `data.recommended_commands[]` when the result exposes exact follow-up commands.
7. `artifacts.artifact_root` as the fallback evidence pointer; run `replay packet` first so the loop has `run-summary.json` and `run-summary.md`, and use `artifacts open` only when you specifically need the generic artifact browser.
