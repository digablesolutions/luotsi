...
4. `artifacts.artifact_root` as the fallback evidence pointer; run `replay packet` first so the loop has `run-summary.json` and `run-summary.md`, and use `artifacts open` only when you specifically need the generic artifact browser.
