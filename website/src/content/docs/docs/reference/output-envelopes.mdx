...
The source tree includes executable examples of this priority order at `examples/agents/extract-next-command.py` and `examples/agents/extract-next-command.mjs`. The same parsers accept persisted `run-summary.json` packets with schema `luotsi-run-summary.v1`; the source contract is documented at `docs/schemas/luotsi-run-summary-v1.md`.
