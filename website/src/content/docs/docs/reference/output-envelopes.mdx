...
3. `data.triage_checklist[].command` or `data.triageChecklist[].command` when a packet/check result exposes a structured first-minute checklist.
4. `data.recommended_next_steps[]`, `data.next_actions[]`, or `data.suggested_commands[]` when the workflow returns an ordered handoff.
5. `data.commands[]`, `data.artifact_commands[]`, or `data.recommended_commands[]` when the result exposes exact follow-up commands.
6. `artifacts.artifact_root` as the fallback evidence pointer; run `replay packet` first so the loop has `run-summary.json` and `run-summary.md`, and use `artifacts open` only when you specifically need the generic artifact browser.
