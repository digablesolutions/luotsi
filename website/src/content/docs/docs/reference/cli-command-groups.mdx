...
Use `journey-intake validate` when Android CLI Journey-style intent needs a CI-friendly contract check before Luotsi scenario drafting. Use `journey-intake draft-scenario` when that valid intake should become a review-required evidence skeleton without touching a device.
...
Use `artifacts list` when you need to rediscover a recent run id. Use `artifacts info` when you want a non-mutating summary of one root or package zip. Use `artifacts open` or `artifacts open --last` when you want the generic artifact browser. Use `artifacts verify --require-lab-safe` when you received a package and need a read-only manifest/SHA/redaction gate before unpacking. Use `artifacts intake --require-lab-safe --write-json --write-readme --sha256 <digest>` when you are ready to validate, restore, and persist an intake audit in one step. Use `replay open` or `replay open --last` when you want the replay-specific front door with next actions.

Successful `artifacts pack` results recommend unpacking with `--sha256`. Lab-safe pack/info/verify handoffs preserve `--require-lab-safe` on unpack commands so CI or support can enforce the gate at extraction time. `artifacts info <package.zip>` validates the manifest, reports redaction metadata and SHA-256, and suggests checksum-verified unpack/replay commands without extracting files.
