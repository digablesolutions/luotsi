...
Use `scenario-validate` or `scenario-explain` before you run. Reach for `replay` commands after a failure when the device is gone but the artifact root remains.
