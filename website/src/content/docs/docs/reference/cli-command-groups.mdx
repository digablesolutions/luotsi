...
Use `artifacts list` when you need to rediscover a recent run id. Use `artifacts info` when you want a non-mutating summary of one root or package zip; restored roots with an intake audit include compact status, share-safety, and SHA verification fields. Use `artifacts open` or `artifacts open --last` when you want the generic artifact browser. Use `artifacts verify --require-lab-safe --sha256 <digest>` when you received a package and need a read-only manifest/SHA/redaction gate before unpacking. Use `artifacts intake --require-lab-safe --write-json --write-readme --sha256 <digest>` when you are ready to validate, restore, and persist an intake audit in one step. Use `replay open` or `replay open --last` when you want the replay-specific front door with next actions.
