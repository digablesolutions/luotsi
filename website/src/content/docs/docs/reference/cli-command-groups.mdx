...
Successful `artifacts pack` results recommend unpacking with `--sha256`. Lab-safe pack/info/verify handoffs preserve `--require-lab-safe` on unpack commands so CI or support can enforce the gate at extraction time. `artifacts info <package.zip>` validates the manifest, reports redaction metadata and SHA-256, and suggests checksum-verified unpack/replay/capsule commands without extracting files.
