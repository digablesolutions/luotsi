...
- `lab status`, `lab doctor`, `lab plan`, `lab claim`, `lab leases`, `lab release`, `lab extend`, `lab quarantine`, `lab quarantines`, `lab unquarantine`
...
Use `scenario-validate`, `scenario-explain`, or `run --dry-run` before you execute device work. Use `artifacts list` when you need to rediscover a recent run id, `artifacts info` when you want a non-mutating summary of one bundle, `artifacts open` when you want the generic artifact browser, and `replay open` when you want the replay-specific front door with next actions. Reach for `replay capsule` when one artifact root failed, `replay graph` when you need selector/action/telemetry context, and `replay cluster` when the same failure pattern is repeating across runs.
