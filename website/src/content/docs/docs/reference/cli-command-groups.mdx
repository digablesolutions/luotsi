...
`doctor` returns a `readiness_plan` with status, blockers, `next_command`, and `recommended_commands`, so the first-run report doubles as an operator/agent handoff.
