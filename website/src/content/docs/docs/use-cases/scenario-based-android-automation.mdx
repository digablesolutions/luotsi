...
Use `inspect` first when the flow is still changing. Use scenarios when the next goal is repeatability. After a run, start with `replay open --dry-run` to get the primary failure, recommended next action, and suggested follow-up commands before choosing a deeper replay tool.
