...
This progression keeps the agent loop on the host machine: explore first, codify second, replay after the run. Start replay with `open --dry-run` so the agent gets the primary failure, recommended next action, and follow-up commands before deciding whether to summarize, search, draft, or rerun.
