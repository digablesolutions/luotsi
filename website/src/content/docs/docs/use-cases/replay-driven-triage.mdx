...
Start with `replay open --dry-run --write-json --write-markdown` when the first job is orientation: it returns the primary failure, one recommended next action, and the command set for deeper replay tools without launching the local browser. It also writes `run-summary.json` and `run-summary.md`, which are the stable packet files to attach to CI, reviews, or an agent loop before anyone opens the raw artifact browser.
