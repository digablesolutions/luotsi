...
It writes a discovery map, JSONL event trace, and review-required scenario candidate into the artifact root. Discovery follows changed screens up to `--max-depth` and records the backtracks needed to return through the explored branch. Treat the generated scenario as a starting point: validate and edit it before using it as CI coverage.
