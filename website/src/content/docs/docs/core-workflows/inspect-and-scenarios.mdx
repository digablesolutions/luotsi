...
Scenario runs and long-lived sessions write artifact roots on purpose. Once a flow has been codified, use `luotsi artifacts list` to rediscover recent runs, then start replay-specific triage with `replay open`, `replay summarize`, or `replay timeline` after the device session ends. Use `artifacts open` only when you specifically need the generic browser.
