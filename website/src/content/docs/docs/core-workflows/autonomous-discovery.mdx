...
Discovery starts the target package unless `--no-start` is supplied, reads screen state, ranks visible clickable elements deterministically, skips obvious destructive labels, taps one safe action at a time, records the resulting transition, and backtracks when the screen changes.

Useful bounds:

```bash
luotsi discover --device <serial> --package <app.id> --activity .MainActivity --budget 10m --max-actions 40 --output-dir artifacts
```

Stop reasons include `no_new_actions`, `budget_expired`, `action_limit_reached`, `action_failed`, `screen_state_failed`, and `backtrack_failed`.
