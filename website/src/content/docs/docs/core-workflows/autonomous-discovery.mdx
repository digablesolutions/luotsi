...
Start with `replay open --dry-run` after discovery so Luotsi returns the primary evidence, recommended next action, and follow-up replay commands without launching a browser. Use `artifacts open --last` only when you specifically need to browse every captured file.
