...
3. Packet checklist commands such as `data.triage_checklist[].command` or `data.triageChecklist[].command`
4. Ordered handoff arrays such as `data.recommended_next_steps`, `data.next_actions`, and `data.suggested_commands`
5. Command arrays such as `data.commands`, `data.artifact_commands`, and `data.recommended_commands`
