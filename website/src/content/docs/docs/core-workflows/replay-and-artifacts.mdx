...
- `replay open`: `replay open` is the canonical first stop for one artifact root; returns session counts, primary failure, recommended next action, and follow-up commands before raw artifact browsing, then opens the browser only when `--dry-run` is not supplied
