...
- `replay scrub`: local previous/focused/next event view with exact commands to move through replay evidence
...
`replay graph` returns the promoted graph for one artifact root with `node_count`, `edge_count`, `node_kinds`, `edge_kinds`, optional `json_path` and `markdown_path`, plus the full `nodes` and `edges` arrays. The promoted node kinds that matter most in practice are `failure`, `action`, `selector`, `screen_state`, `telemetry_signal`, `scenario_draft`, and `generated_step`. On this branch it does not narrow by query, so use `replay timeline`, `replay scrub`, or `replay search` to isolate the failure window before you read the graph.

`replay cluster` works one level up. Point it at a directory of replayable artifact roots when you need Luotsi to group failed sessions by normalized failure shape and return triage hints plus the next replay/search commands for the latest matching bundle.
...
4. Use `replay graph` when `replay timeline`, `replay scrub`, or `replay search` has already narrowed the question and you now need promoted selectors/actions/telemetry, or `replay cluster` when the question is whether the same failure shape is repeating across runs.
5. Open the local browser index when you want screenshots, logs, replay markdown, and draft review artifacts in one place.
...
- `scenario-draft.md` lists review items, source summaries, normalizations, step origins, and the draft steps.
