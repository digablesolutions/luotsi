...
`replay graph` returns a stable query/result shape with `query`, `node_count`, `edge_count`, `node_kinds`, `edge_kinds`, `insights`, and `actions`. The promoted node kinds that matter most in practice are `failure`, `action`, `selector`, `screen_state`, `telemetry_signal`, `scenario_draft`, and `generated_step`. Filtering with `--failed`, `--node-kind`, `--edge-kind`, `--action`, or `--selector` keeps one-hop context around the returned slice so the graph stays readable instead of dumping the whole root.
...
4. Use `replay graph` when you need promoted selectors/actions/telemetry or `replay cluster` when the question is whether the same failure shape is repeating across runs.
