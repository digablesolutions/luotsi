...
- `artifacts verify`: validate a received package manifest, SHA-256, redaction status, and exact unpack/replay/capsule commands without writing files; add `--require-lab-safe` when CI or support must reject unredacted packages before unpacking
