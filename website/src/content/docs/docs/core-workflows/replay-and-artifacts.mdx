...
- `run-summary.json` and `run-summary.md` are the stable packet files to attach after `replay packet`; the Markdown starts with an At a Glance summary, failure snapshot, packet gate, copy-paste triage commands, and the 60-second checklist, and the source contract is documented in `docs/schemas/luotsi-run-summary-v1.md`
