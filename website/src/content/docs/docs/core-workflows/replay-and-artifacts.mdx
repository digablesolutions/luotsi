...
- `replay packet`: write `run-summary.json` and `run-summary.md`, return the `luotsi-run-summary.v1` packet, and stay non-interactive for CI and agents; use `--last` when the agent only has a search root
