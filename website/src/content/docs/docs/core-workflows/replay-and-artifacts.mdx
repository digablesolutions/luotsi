...
luotsi artifacts verify my-run-lab-safe.zip
...
- `artifacts verify`: validate a received package manifest, SHA-256, redaction status, and exact unpack commands without writing files
...
`artifacts pack` writes `luotsi-artifact-package.json` into the zip with the run id, created timestamp, category counts, and recommended reopen commands. By default the package is an exact copy; `--redact lab-safe` redacts obvious secrets from text-like zip entries, leaves source artifacts and binary media unchanged, and records redaction counts in the manifest. `artifacts verify` validates the manifest and archive entries without extracting the package, reports SHA-256 plus `share_safety`, and returns exact unpack/replay commands. `artifacts unpack` requires the same manifest, reports it back in the result, and both pack and unpack support `--dry-run` when you want to validate the handoff without writing files yet.
