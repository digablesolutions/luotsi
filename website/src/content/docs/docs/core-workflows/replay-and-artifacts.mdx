...
`artifacts verify` validates the manifest and archive entries without extracting the package. It reports SHA-256 plus `share_safety` and returns exact unpack/replay/capsule commands. With `--require-lab-safe`, unredacted packages return `status: blocked`, blockers, repair commands, and exit code 1 before unpacking.
