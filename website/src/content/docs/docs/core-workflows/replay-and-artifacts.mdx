...
- `run-summary.json` and `run-summary.md` are the stable packet files to attach to CI, reviews, and agent loops after `replay packet`; the Markdown starts with copy-paste triage commands followed by the 60-second checklist, and the source contract is documented in `docs/schemas/luotsi-run-summary-v1.md`
