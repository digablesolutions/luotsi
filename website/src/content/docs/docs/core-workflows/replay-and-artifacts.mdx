...
- `replay packet`: write `run-summary.json` and `run-summary.md`, return the `luotsi-run-summary.v1` packet, and stay non-interactive for CI and agents; use `--human` when a terminal reader needs the same first-minute triage path inline, use `--check` to validate an existing packet without rewriting artifacts, and use `--last` when the agent only has a search root
