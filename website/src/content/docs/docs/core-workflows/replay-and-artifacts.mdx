...
luotsi artifacts unpack my-run-lab-safe.zip --output ./artifacts/my-run --sha256 <digest>
...
`artifacts pack` writes `luotsi-artifact-package.json` into the zip with the run id, created timestamp, category counts, and recommended reopen commands. By default the package is an exact copy; `--redact lab-safe` redacts obvious secrets from text-like zip entries, leaves source artifacts and binary media unchanged, and records redaction counts in the manifest. Successful pack results recommend unpacking with `--sha256`. `artifacts verify` validates the manifest and archive entries without extracting the package, reports SHA-256 plus `share_safety`, and returns exact unpack/replay commands. With `--require-lab-safe`, unredacted packages return `status: blocked`, blockers, repair commands, and exit code 1 before unpacking. `artifacts info <package.zip>` offers the same non-mutating package intake through the existing info workflow. `artifacts unpack` requires that manifest, rejects `--sha256` mismatches before writing files, reports the manifest back in the result, and both pack and unpack support `--dry-run` when you want to validate the handoff without writing files yet.
