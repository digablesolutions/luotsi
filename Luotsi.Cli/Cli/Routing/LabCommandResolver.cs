...
internal static class LabCommandResolver
{
    ...
    public static async Task<LabDoctorResult> DiagnoseAsync(IDeviceHost runner, string? query)
    {
    ...
    private static LabDeviceDecision ToDecision(DeviceState device, DeviceQuery? selector)
    {
        ...
        return new LabDeviceDecision(device.Serial, device.State, reason, selected);
    }
}
