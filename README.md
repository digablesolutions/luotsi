...
5. **Replay failures** - start with `luotsi replay packet --artifacts <artifact-root>` to write `run-summary.json` and `run-summary.md` with the failure snapshot, packet gate, and first-minute commands. Validate shared packets with `luotsi replay packet --artifacts <artifact-root> --check`, then use `replay open --dry-run`, `summarize`, `capsule`, `timeline`, `scrub`, `graph`, `search`, `scenario-draft`, and clustering from the same saved artifacts.
