...
Markdown checklist contains the structured checklist actions, that the
`## First Action` section repeats the recommended action title, kind, reason,
and command, and that the `## Primary Failure` section carries the detailed
primary failure values or explicitly says that no primary failure was found.
