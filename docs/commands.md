...
When `doctor --device <serial> --package <app.id>` reports `readiness_plan.status: ready`, the `next_command` now routes to `discover` first so a ready app immediately becomes review-required scenario candidates. The same `recommended_commands` list still includes inspect, screen-state, persisted quickstart handoff, scenario validation, run, and live view commands.
...
| `artifacts intake <artifact.zip> [--output <directory>] [--force] [--dry-run] [--require-lab-safe] [--sha256 <digest>] [--write-json] [--write-readme] [--open]` | One-step received-package handoff for support, CI, and agents; applies the same package validation and lab-safe/SHA gates as unpack, restores the artifact root, returns exact info/open/replay commands, can persist schema-backed `artifact-intake-summary.json` / `artifact-intake.md`, and can open the refreshed index after a successful restore |
