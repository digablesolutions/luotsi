...
**Artifacts.** Use `--artifacts <directory>` to override the artifact root for the current command or session. Use `--poll-artifacts <final|per-attempt|none>` to control whether polling-style commands write artifacts only at the end, on each attempt, or not at all. Use `luotsi artifacts list [--artifacts <directory>] [--limit 20]` to discover recent run ids, `luotsi artifacts info <artifact-root-or-run-id>` to summarize one bundle without opening or changing it, `luotsi artifacts open <artifact-root-or-run-id>` to open or regenerate the local artifact index, `luotsi artifacts pack <artifact-root-or-run-id>` to create a zip for sharing or CI upload, and `luotsi artifacts unpack <artifact.zip>` to restore a shared package locally. `artifacts pack` and `artifacts unpack` both support `--dry-run` for safe handoff previews.
...
| `artifacts pack <artifact-root-or-run-id> [--output <file.zip>] [--force] [--dry-run]` | Pack an artifact root into a zip with relative entries for sharing, upload, or local handoff; successful writes report SHA-256, and `--dry-run` reports the output path and entry count without writing |
| `artifacts unpack <artifact.zip> [--output <directory>] [--force] [--dry-run]` | Extract a packed artifact bundle into a local root with zip-slip protection and package SHA-256 reporting; `--dry-run` validates entries without writing |
