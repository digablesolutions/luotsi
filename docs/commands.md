...
**Artifacts.** Use `--artifacts <directory>` to override the artifact root for the current command or session. Use `--poll-artifacts <final|per-attempt|none>` to control whether polling-style commands write artifacts only at the end, on each attempt, or not at all. Use `luotsi artifacts list` to discover recent run ids, `luotsi artifacts open <artifact-root-or-run-id>` to open or regenerate the local artifact index, and `luotsi artifacts pack <artifact-root-or-run-id>` to create a zip for sharing or CI upload.
