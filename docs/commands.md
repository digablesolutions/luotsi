...
**Artifacts.** Use `--artifacts <directory>` to override the artifact root for the current command or session; scenario `run` also accepts `--output-dir <directory>` as a clearer alias for the same root. Use `--poll-artifacts <final|per-attempt|none>` to control whether polling-style commands write artifacts only at the end, on each attempt, or not at all. Use `luotsi artifacts list [--artifacts <directory>] [--limit 20]` to discover recent run ids, `luotsi artifacts info <artifact-root-or-run-id>` to summarize one bundle without opening or changing it, `luotsi artifacts open <artifact-root-or-run-id>` to open or regenerate the local artifact index, `luotsi artifacts pack <artifact-root-or-run-id>` to create a zip for sharing or CI upload, and `luotsi artifacts unpack <artifact.zip>` to restore a shared package locally. `artifacts pack` writes `luotsi-artifact-package.json` into the zip with schema/version, run id, created timestamp, category counts, recommended unpack-time commands, and packed file paths. `artifacts unpack` requires that manifest, reports it back in the command result, and refreshes `index.html` for non-dry-run restores so shared zips can be opened immediately. `artifacts pack` and `artifacts unpack` both support `--dry-run` for safe handoff previews.
