...
**Artifacts.** Use `--artifacts <directory>` to override the artifact root for the current command or session; scenario `run` also accepts `--output-dir <directory>` as a clearer alias for the same root. Without an explicit override, `run` writes into Luotsi's default user-local run-artifact home (`%LOCALAPPDATA%\Luotsi\artifacts` on Windows, `~/.local/share/luotsi/artifacts` on Linux/macOS, with temp-folder fallback when those locations are unavailable). Use `--poll-artifacts <final|per-attempt|none>` to control whether polling-style commands write artifacts only at the end, on each attempt, or not at all. Use `luotsi artifacts list [--artifacts <directory>] [--limit 20]` to discover recent run ids, `luotsi artifacts info <artifact-root-or-run-id>` or `luotsi artifacts info --last [--artifacts <directory>]` to summarize one bundle without opening or changing it, `luotsi artifacts open <artifact-root-or-run-id>` or `luotsi artifacts open --last [--artifacts <directory>]` to open or regenerate the local artifact index, `luotsi artifacts pack <artifact-root-or-run-id>` to create a zip for sharing or CI upload, and `luotsi artifacts unpack <artifact.zip>` to restore a shared package locally. `replay open` is the fastest human re-entry point: use `luotsi replay open --artifacts <artifact-root>` for an explicit bundle or `luotsi replay open --last [--artifacts <directory>]` to resume the latest local triage root. `artifacts pack` writes `luotsi-artifact-package.json` into the zip with schema/version, run id, created timestamp, category counts, recommended unpack-time commands, and packed file paths. By default packages are exact copies; `--redact lab-safe` opt-in redacts obvious secrets from text-like entries while leaving source artifacts and binary media unchanged, and records redaction counts in the manifest. `artifacts unpack` requires that manifest, reports it back in the command result, and refreshes `index.html` for non-dry-run restores so shared zips can be opened immediately. `artifacts pack` and `artifacts unpack` both support `--dry-run` for safe handoff previews.
