...
- Validation and safety gates:
  ...
  - `artifacts verify` and `artifacts info <package.zip>` require the manifest, check archive entries against it, report SHA-256 plus lab-safe redaction status, and return checksum-verified unpack plus `replay packet --check` handoff commands without extracting.
...
- Restore workflows:
  ...
  - `artifacts intake` also reports received-package status and exact info/packet-check/open/replay commands, supports `--open` to launch the refreshed index, and can persist `artifact-intake-summary.json` and `artifact-intake.md` with `--write-json --write-readme`.
...
| `artifacts info (<artifact-root-or-run-id-or-package.zip> \| --last [--artifacts <directory>])` | Summarize one artifact root or run id, including index/replay flags, restored package intake audit when present, file categories, and next commands, without mutating the bundle; when given a package zip, validate the manifest and report redaction metadata, SHA-256, and unpack/packet-check/replay commands without extracting files; `--last` resolves the latest root under the default Luotsi run-artifact home or `--artifacts <directory>` |
...
| `artifacts intake <artifact.zip> [--output <directory>] [--force] [--dry-run] [--require-lab-safe] [--sha256 <digest>] [--write-json] [--write-readme] [--open]` | One-step received-package handoff for support, CI, and agents; applies the same package validation and lab-safe/SHA gates as unpack, restores the artifact root, returns exact info/packet-check/open/replay commands, can persist schema-backed `artifact-intake-summary.json` / `artifact-intake.md`, and can open the refreshed index after a successful restore |
...
| `replay capsule --artifacts <artifact-root> [--write-readme] [--write-json]` | Write the replay capsule with session counts, primary failure, artifact counts, intake audit/package safety summary when present, recommended next steps, and suggested commands |
...
| `replay open (--artifacts <artifact-root> \| --last [--artifacts <directory>]) [--dry-run] [--write-json] [--write-markdown]` | Return the canonical replay front-door summary with session counts, primary failure, recommended next action, and follow-up commands before raw artifact browsing; refreshes the artifact index and, without `--dry-run`, opens `index.html` locally; `--last` reopens the latest local triage bundle without re-copying the path |
| `replay packet (--artifacts <artifact-root> \| --last [--artifacts <directory>]) [--check]` | Write `run-summary.json` and `run-summary.md`, return the `luotsi-run-summary.v1` packet with status, failure snapshot, focused evidence files, recommended next action, 60-second checklist, and follow-up commands, and avoid launching a browser; use this when CI or an agent wants the durable investigation packet directly; `--human` prints the same first-minute triage path inline; `--check` validates an existing packet without rewriting artifacts and returns the same next action/evidence/checklist handoff for continuation; `--last` resolves the latest local triage bundle |
...
- `run-summary.json` uses the `luotsi-run-summary.v1` contract documented in `docs/schemas/luotsi-run-summary-v1.md`; use `replay packet --check` as the CI/agent gate for an existing packet. The gate verifies the Markdown companion, failure snapshot, focused evidence files, packet gate, copy-paste commands, checklist, first action, primary failure details, and command section. `replay packet --human` and `replay packet --check --human` print status, primary failure evidence, evidence files, the best next command, a `copy_paste` block that starts with the packet validation gate command, checklist commands, and packet paths without requiring a JSON reader.
- `artifacts info` is the non-mutating check for one artifact root or package zip:
  ...
  - For package zips, it validates the manifest and entries, reports redaction metadata plus SHA-256, and returns checksum-verified unpack plus `replay packet --check` commands without creating files.
...
- `artifacts intake` is the one-command restore path for received packages. It performs the same validation, reports `status` (`validated` or `restored`), can launch the refreshed index with `--open`, can write `artifact-intake-summary.json` with schema `luotsi-artifact-intake.v1` plus `artifact-intake.md`, and returns exact next commands for info, packet validation, open, and replay. `replay capsule` surfaces those intake files and compact status/share-safety/SHA verification fields when they are present in the restored root.
...
- `--output-dir <directory>` is a scenario-run alias for `--artifacts <directory>`. Successful run results include `artifact_commands` with exact `replay open`, `artifacts open`, and `artifacts pack` commands for the run artifact root.
...
Source checkouts include executable parser examples at `examples/agents/extract-next-command.py` and `examples/agents/extract-next-command.mjs`. They accept one JSON envelope or a saved JSONL-style log and print the best next command.
