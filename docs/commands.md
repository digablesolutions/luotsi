...
| `replay packet (--artifacts <artifact-root> \| --last [--artifacts <directory>])` | Write `run-summary.json` and `run-summary.md`, return the `luotsi-run-summary.v1` packet, and avoid launching a browser; use this when CI or an agent wants the durable investigation packet directly; `--last` resolves the latest local triage bundle |
