...
| `artifacts info (<artifact-root-or-run-id> \| --last [--artifacts <directory>])` | Summarize one artifact root or run id, including index/replay flags, file categories, and next commands, without mutating the bundle; `--last` resolves the latest root under the default temp artifact root or `--artifacts <directory>` |
| `artifacts open (<artifact-root-or-run-id> \| --last [--artifacts <directory>]) [--dry-run]` | Refresh/open the artifact index for a root or run id; run-id lookup and `--last` resolution search the default temp artifact root or `--artifacts <directory>` |
