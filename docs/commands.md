...
| `artifacts pack <artifact-root-or-run-id> [--output <file.zip>] [--force] [--dry-run]` | Pack an artifact root into a zip with relative entries plus `luotsi-artifact-package.json` for sharing, upload, or local handoff; successful writes report SHA-256, and `--dry-run` reports the output path, manifest, and entry count without writing |
...
| `replay scenario-draft --artifacts <artifact-root> --output <scenario.json> [--name <name>] [--validate] [--write-json] [--write-markdown]` | Convert inspect/replay action events into a conservative starter scenario with review items, warnings, inserted pre-tap waits, cleanup normalizations, and optional immediate static validation |
