...
**Artifacts.** Use `--artifacts <directory>` to override the artifact root for the current command or session; scenario `run` also accepts `--output-dir <directory>` as a clearer alias for the same root. Without an explicit override, `run` writes into Luotsi's default user-local run-artifact home (`%LOCALAPPDATA%\Luotsi\artifacts` on Windows, `~/.local/share/luotsi/artifacts` on Linux/macOS, with temp-folder fallback when those locations are unavailable). Use `--poll-artifacts <final|per-attempt|none>` to control whether polling-style commands write artifacts only at the end, on each attempt, or not at all. Use `luotsi artifacts list [--artifacts <directory>] [--limit 20]` to discover recent run ids, `luotsi artifacts info <artifact-root-or-run-id>` or `luotsi artifacts info --last [--artifacts <directory>]` to summarize one bundle without opening or changing it, `luotsi artifacts open <artifact-root-or-run-id>` or `luotsi artifacts open --last [--artifacts <directory>]` to open or regenerate the local artifact index, `luotsi artifacts pack <artifact-root-or-run-id>` to create a zip for sharing or CI upload, `luotsi artifacts verify <artifact.zip>` to validate a received package without writing files, and `luotsi artifacts unpack <artifact.zip>` to restore a shared package locally. `replay open` is the fastest human re-entry point: use `luotsi replay open --artifacts <artifact-root>` for an explicit bundle or `luotsi replay open --last [--artifacts <directory>]` to resume the latest local triage root. `artifacts pack` writes `luotsi-artifact-package.json` into the zip with schema/version, run id, created timestamp, category counts, recommended unpack-time commands, and packed file paths. By default packages are exact copies; `--redact lab-safe` opt-in redacts obvious secrets from text-like entries while leaving source artifacts and binary media unchanged, and records redaction counts in the manifest. `artifacts verify` requires that manifest, checks archive entries against it, reports SHA-256 plus lab-safe redaction status, and returns exact unpack/replay commands without extracting the package. Use `--require-lab-safe` to make verification an enforcing CI/support handoff gate; unredacted packages return `status: blocked`, list blockers, and exit non-zero before anyone unpacks them. `artifacts unpack` performs the same validation, reports the manifest back in the command result, and refreshes `index.html` for non-dry-run restores so shared zips can be opened immediately. `artifacts pack` and `artifacts unpack` both support `--dry-run` for safe handoff previews.
...
| `artifacts info (<artifact-root-or-run-id> \| --last [--artifacts <directory>])` | Summarize one artifact root or run id, including index/replay flags, file categories, and next commands, without mutating the bundle; `--last` resolves the latest root under the default Luotsi run-artifact home or `--artifacts <directory>` |
...
| `artifacts unpack <artifact.zip> [--output <directory>] [--force] [--dry-run]` | Extract a packed artifact bundle into a local root with zip-slip protection, required manifest validation, and package SHA-256 reporting; `--dry-run` validates entries and the manifest without writing |
...
`artifacts list` is the discovery step for local runs: it returns run ids, roots, file counts, index presence, replay metadata presence, and exact info/open/pack commands. By default it searches Luotsi's user-local run-artifact home, which is also where `run` writes artifacts when you do not pass `--artifacts` or `--output-dir`. `artifacts info` is the non-mutating check for one artifact root: it resolves either a path or run id, or uses `--last` to reopen the latest root under that default run-artifact home or `--artifacts <directory>`, then reports index/replay flags plus screenshot/video/report/log/timeline counts and exact open/pack/replay commands. `artifacts open` is the generic artifact entry point when you have a root, a copied run id, or just want the latest local triage root. It refreshes `index.md`/`index.html` if needed and opens the best available index. `artifacts pack` writes a zip with relative paths, reports the package SHA-256, and refuses to overwrite unless `--force` is supplied; `--dry-run` previews the output path and entry count without creating the zip. With `--redact lab-safe`, pack redacts text-like entries such as JSON, JSONL, XML, TXT, LOG, Markdown, HTML, CSV, properties, and env files while copying binary media byte-for-byte and never modifying the source artifact root. `artifacts verify` validates a package without writing files, reports the source package SHA-256, shows `share_safety` (`lab_safe` or `not_redacted`), checks every archive entry against the manifest, and returns exact unpack/info/replay commands. With `--require-lab-safe`, verify still reports the package but returns `status: blocked`, `lab_safe_required: true`, blockers, redaction-repair commands, and exit code 1 when `share_safety` is `not_redacted`. `artifacts unpack` extracts that zip into a local artifact root, reports the source package SHA-256, and rejects entries that would escape the output directory; `--dry-run` performs the same package validation without creating files. `replay open` remains the canonical replay front door. It attaches the existing artifact root, or resolves the latest local triage root with `--last`, regenerates `index.md` and `index.html`, opens the HTML index with the platform default opener, and returns session/failure counts, primary failure, one recommended next action, and commands into capsule, timeline, scrub, graph, search, scenario draft, and clustering. In `--human` mode that front door now becomes a capsule-style terminal summary: Luotsi surfaces the failure signal count, the primary failure, the next step title, and the best next command before listing the longer follow-up command set. `--dry-run` returns the same front-door summary plus the opener command without launching it. `--write-json` and `--write-markdown` persist `replay-open-summary.json` and `replay-open.md` into the artifact root. `replay summarize` returns the normal JSON command envelope by default and now includes summary-level `commands` that point into replay open, replay capsule, failure scrub, failure graph, and repeated-failure clustering. `--format json` writes only the replay summary object. `--format jsonl` writes a `type: summary` header line followed by one `type: session` line per replay session. Failed scenario runs expose `failure_capsule_path` plus an embedded `failure_capsule` summary with linked reports, grouped failure artifacts, and failure-bundle metadata. `replay capsule` writes the higher-level bundle capsule: it identifies the primary failure with an exact reopen command, includes a compact `failure_timeline` with exact `replay timeline` source commands, reports existing `scenario_draft_artifacts` and `scenario_draft_summary` including bounded draft next actions when present, promotes ready draft dry-run, preflight, claimed-run, and direct-run handoff commands before other draft-led `recommended_next_steps`, counts screenshots/videos/logs/reports/timelines, and returns suggested follow-up commands. With `--write-readme`, it writes `replay-capsule.md` into the artifact root; with `--write-json`, it writes `replay-capsule-summary.json`. Both write options refresh the artifact index, and `--human` mode condenses that data into the same capsule-style summary before printing follow-up commands. `replay timeline` reads `session-timeline.jsonl` files directly and returns ordered events with path, sequence, timestamp, type, failure relevance, detail text, flattened scalar properties, and commands back into open/capsule/scrub/graph when the selected events include failures; `--contains` filters normalized event type/detail text, `--source-path` and `--sequence` reopen exact events referenced by scenario-draft provenance, `--since` and `--until` filter by event timestamp, `--context` includes neighboring events around filtered matches, `--format json|jsonl` writes raw timeline output instead of the command envelope, and `--write-json`, `--write-jsonl`, and `--write-markdown` persist `replay-timeline.json`, `replay-timeline.jsonl`, and `replay-timeline.md` into the artifact root. `replay scrub` uses the same timeline filters but returns a previous/focused/next event view with exact commands to reopen the focused event, move to adjacent events, search the focused detail, open the replay front door, write the replay capsule, or open semantic graph context for focused failures; `--write-json` and `--write-markdown` persist `replay-scrub.json` and `replay-scrub.md`. `replay graph` builds a stable semantic-debug graph and can persist `replay-graph.json`, `replay-graph.jsonl`, and `replay-graph.md`; with `--format json|jsonl`, it writes raw machine output instead of the command envelope. The graph result includes `query`, `taxonomy`, `agent_summary`, `total_node_count`, `total_edge_count`, `matched_node_count`, `matched_edge_count`, `truncated`, `node_kinds`, `edge_kinds`, `insights`, `actions`, `evidence_kinds`, `evidence`, `failure_paths`, promoted semantic nodes, and `transitions_to` edges that classify timeline movement such as action-to-failure, screen changes, and telemetry observations. Graph actions start with replay open so agents can move from semantic context back to the canonical front door. Use `--failed`, `--node-kind`, `--edge-kind`, `--action`, `--selector`, `--contains`, `--insight`, `--severity`, `--evidence`, and `--limit` to return focused graph, insight, and evidence context for agents; use `--node <id> --depth <n>` to expand a deterministic neighborhood around a specific graph node. `replay cluster` groups failed replay sessions by normalized failure shape, including error category/message and failed action/step, returns triage hints with suggested open/capsule/graph/scrub/search commands for the best matching bundle, and can persist `replay-clusters.json` and `replay-clusters.md`. `replay scenario-draft` reads timeline action events from inspect/view/replay artifacts and writes a valid JSON scenario draft when enough action data is available; generated taps receive matching pre-tap waits when the previous step does not already wait for the same text or selector. `--validate` runs static scenario validation immediately after writing `--output`/`--file` and records a compact `validation` object without running a device scenario. When timeline evidence includes a trusted package or focus field, the draft records `package_provenance`; when replay metadata has one trusted adb target, it records `device_provenance`. Those values stamp scenario metadata and make validated preflight/run handoff commands concrete when available. Validated drafts also expose `run_handoff` with `luotsi run --path <draft> --dry-run`, app preflight, direct run, and, when trusted package plus concrete device serial are available, `--claim-device --claim-wait-sec 60` claimed-run commands; unvalidated or failed drafts report why the run handoff is gated. `--write-json` and `--write-markdown` persist draft review artifacts with confidence, warnings, suggestions, typed `next_actions`, package/device provenance when available, validation status when requested, run handoff status including claimed-run command when concrete, normalizations, exact source file/sequence provenance, matching `replay timeline` source commands, capsule/open/scrub/search commands, and a graph command for draft provenance. `replay search` scans JSON, JSONL, XML, text, log, Markdown, HTML, and CSV artifacts and returns relative file paths with line numbers, previews, and commands back into open/capsule/scrub/graph when relevant. Failures continue to use the normal error envelope.
