...
| `replay packet (--artifacts <artifact-root> \| --last [--artifacts <directory>]) [--check]` | Write `run-summary.json` and `run-summary.md`, return the `luotsi-run-summary.v1` packet, and avoid launching a browser; use this when CI or an agent wants the durable investigation packet directly; `--human` prints the same first-minute triage path inline; `--check` validates an existing packet without rewriting artifacts; `--last` resolves the latest local triage bundle |
...
- `run-summary.json` uses the `luotsi-run-summary.v1` contract documented in `docs/schemas/luotsi-run-summary-v1.md`; use `replay packet --check` as the CI/agent gate for an existing packet. `replay packet --human` and `replay packet --check --human` print status, primary failure evidence, the best next command, checklist commands, and packet paths without requiring a JSON reader.
