...
public sealed partial class AppTests
{
    ...
    public async Task RunAsync_Help_Command_Writes_Artifacts_Topic()
    {
        ...
        Assert.Contains("luotsi artifacts info (<artifact-root-or-run-id-or-package.zip> | --last [--artifacts <directory>])", console.ErrorLines[0], StringComparison.Ordinal);
    ...
    public async Task RunAsync_ReplaySummarize_Returns_Condensed_Replay_Summary()
    {
        ...
        Assert.Equal("write_replay_packet", commands[0].GetProperty("kind").GetString());
        Assert.Equal($"luotsi replay packet --artifacts {replayRoot}", commands[0].GetProperty("command").GetString());
        Assert.Equal("check_replay_packet", commands[1].GetProperty("kind").GetString());
    ...
    public async Task RunAsync_ReplaySummarize_FormatJsonl_Writes_Summary_And_Session_Lines()
    {
        ...
        Assert.Equal("write_replay_packet", summaryLine.RootElement.GetProperty("commands")[0].GetProperty("kind").GetString());
        Assert.Equal("check_replay_packet", summaryLine.RootElement.GetProperty("commands")[1].GetProperty("kind").GetString());
    ...
    public async Task RunAsync_ReplayOpen_WriteArtifacts_PersistsFrontDoorSummary()
    {
        ...
        Assert.Contains("## Packet Gate", runSummaryMarkdown, StringComparison.Ordinal);
        Assert.Contains($"luotsi replay packet --artifacts {replayRoot} --check", runSummaryMarkdown, StringComparison.Ordinal);
        Assert.Contains("## Failure Snapshot", runSummaryMarkdown, StringComparison.Ordinal);
        Assert.Contains("Message: `error=transport: Unexpected end of stream`", runSummaryMarkdown, StringComparison.Ordinal);
    ...
    public async Task RunAsync_ReplayPacket_Check_ValidatesExistingRunSummary()
    {
        ...
        Assert.Equal("needs_triage", data.GetProperty("packet_status").GetString());
        Assert.Equal(replayRoot, data.GetProperty("artifact_root").GetString());
    ...
    public async Task RunAsync_ReplayPacket_Check_MarkdownWithStaleIdentity_ReturnsUsageError()
    {
        ...
        Assert.Contains("Artifact root:", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
        Assert.Contains("Re-run `luotsi replay packet", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
    }
    ...
    [Fact]
    public async Task RunAsync_ReplayPacket_Check_MarkdownWithStaleGeneratedAt_ReturnsUsageError()
    {
        var console = new FakeConsole();
        var fileSystem = new FakeFileSystem();
        var replayRoot = SeedReplaySummaryArtifacts(fileSystem);
        ...
        Assert.Equal("usage_error", envelope.RootElement.GetProperty("error").GetProperty("category").GetString());
        Assert.Contains("packet identity value", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
        Assert.Contains($"Generated: `{generatedAt}`", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
        Assert.Contains("Re-run `luotsi replay packet", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
    }

    [Fact]
    public async Task RunAsync_ReplayPacket_Check_MarkdownWithStaleVerdict_ReturnsUsageError()
    {
        var console = new FakeConsole();
        var fileSystem = new FakeFileSystem();
        var replayRoot = SeedReplaySummaryArtifacts(fileSystem);
        ...
        fileSystem.AddFile(markdownPath, markdown.Replace(
            ...
            "Verdict: No failure signals found.",
            StringComparison.Ordinal));
        console.OutputLines.Clear();
        var checkExitCode = await app.RunAsync(["replay", "packet", "--check", "--artifacts", replayRoot]);
        using var envelope = console.ParseSingleOutputAsJson();
        ...
        Assert.Equal("usage_error", envelope.RootElement.GetProperty("error").GetProperty("category").GetString());
        Assert.Contains("packet identity value", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
        Assert.Contains("Verdict: Failure signals found. Start with the recommended next action before broad artifact browsing.", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
        Assert.Contains("Re-run `luotsi replay packet", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
    }
    ...
    public async Task RunAsync_ReplayPacket_Check_MarkdownWithoutPacketGate_ReturnsUsageError()
    {
        ...
        Assert.False(envelope.RootElement.GetProperty("ok").GetBoolean());
        Assert.Equal("usage_error", envelope.RootElement.GetProperty("error").GetProperty("category").GetString());
        Assert.Contains("packet validation gate command", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
        Assert.Contains("Re-run `luotsi replay packet", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
    }

    [Fact]
    public async Task RunAsync_ReplayPacket_Check_MarkdownWithoutFailureSnapshot_ReturnsUsageError()
    {
        var console = new FakeConsole();
        var fileSystem = new FakeFileSystem();
        var replayRoot = SeedReplaySummaryArtifacts(fileSystem);
    ...
    [Fact]
    public async Task RunAsync_ReplayPacket_Check_PrimaryFailureWithoutSourceCommand_ReturnsUsageError()
    {
        var console = new FakeConsole();
        var fileSystem = new FakeFileSystem();
        var processRunner = new FakeProcessRunner();
        ...
        var packetPath = Path.Join(replayRoot, "run-summary.json");
        var packet = JsonNode.Parse(await fileSystem.ReadAllTextAsync(packetPath))!.AsObject();
        packet["primaryFailure"]!.AsObject()["sourceCommand"] = null;
        fileSystem.AddFile(packetPath, packet.ToJsonString());
        console.OutputLines.Clear();
        var checkExitCode = await app.RunAsync(["replay", "packet", "--check", "--artifacts", replayRoot]);
        using var envelope = console.ParseSingleOutputAsJson();
        ...
        Assert.False(envelope.RootElement.GetProperty("ok").GetBoolean());
        Assert.Equal("usage_error", envelope.RootElement.GetProperty("error").GetProperty("category").GetString());
        Assert.Contains("primaryFailure.sourceCommand is required", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
        Assert.Contains("Re-run `luotsi replay packet", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
        Assert.Empty(processRunner.Calls);
    }

    [Fact]
    public async Task RunAsync_ReplayPacket_Check_PrimaryFailureWithMissingTimeline_ReturnsUsageError()
    {
        var console = new FakeConsole();
        var fileSystem = new FakeFileSystem();
        var processRunner = new FakeProcessRunner();
        ...
        var writeExitCode = await app.RunAsync(["replay", "packet", "--artifacts", replayRoot]);
        fileSystem.DeleteFile(Path.Join(replayRoot, "session-timeline.jsonl"));
        console.OutputLines.Clear();
        var checkExitCode = await app.RunAsync(["replay", "packet", "--check", "--artifacts", replayRoot]);
        using var envelope = console.ParseSingleOutputAsJson();
        ...
        Assert.False(envelope.RootElement.GetProperty("ok").GetBoolean());
        Assert.Equal("usage_error", envelope.RootElement.GetProperty("error").GetProperty("category").GetString());
        Assert.Contains("primaryFailure.timelinePath points at missing evidence file", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
        Assert.Contains("session-timeline.jsonl", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
        Assert.Contains("Re-run `luotsi replay packet", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
        Assert.Empty(processRunner.Calls);
    }

    [Fact]
    public async Task RunAsync_ReplayPacket_Check_PrimaryFailureWithMissingFailureCapsule_ReturnsUsageError()
    {
        var console = new FakeConsole();
        var fileSystem = new FakeFileSystem();
        var processRunner = new FakeProcessRunner();
        ...
        var writeExitCode = await app.RunAsync(["replay", "packet", "--artifacts", replayRoot]);
        fileSystem.DeleteFile(Path.Join(replayRoot, "failure-capsule.json"));
        console.OutputLines.Clear();
        var checkExitCode = await app.RunAsync(["replay", "packet", "--check", "--artifacts", replayRoot]);
        using var envelope = console.ParseSingleOutputAsJson();
        ...
        Assert.False(envelope.RootElement.GetProperty("ok").GetBoolean());
        Assert.Equal("usage_error", envelope.RootElement.GetProperty("error").GetProperty("category").GetString());
        Assert.Contains("primaryFailure.failureCapsulePath points at missing evidence file", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
        Assert.Contains("failure-capsule.json", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
        Assert.Contains("Re-run `luotsi replay packet", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
        Assert.Empty(processRunner.Calls);
    }
    ...
    public async Task RunAsync_ReplayPacket_Check_MissingPacket_ReturnsUsageError()
    {
        ...
        var exitCode = await app.RunAsync(["replay", "packet", "--check", "--artifacts", replayRoot]);
        using var envelope = console.ParseSingleOutputAsJson();

        Assert.Equal(2, exitCode);
        Assert.False(envelope.RootElement.GetProperty("ok").GetBoolean());
        Assert.Equal("usage_error", envelope.RootElement.GetProperty("error").GetProperty("category").GetString());
        Assert.Contains("requires run-summary.json", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
        Assert.Contains("luotsi replay packet", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
    }

    [Fact]
    public async Task RunAsync_ReplayPacket_Check_StaleArtifactRoot_ReturnsUsageError()
    {
        var console = new FakeConsole();
        var fileSystem = new FakeFileSystem();
        var replayRoot = SeedReplaySummaryArtifacts(fileSystem);
        var staleRoot = Path.Join("/tmp", "other-run");
        var staleJsonPath = Path.Join(staleRoot, "run-summary.json");
        var staleMarkdownPath = Path.Join(staleRoot, "run-summary.md");
        fileSystem.CreateDirectory(staleRoot);
        fileSystem.AddFile(staleMarkdownPath, "# Luotsi Run Summary");
        fileSystem.AddFile(staleJsonPath, JsonSerializer.Serialize(new
        {
            ...
            entryPoints = new
            {
                ...
                runSummaryJsonPath = staleJsonPath,
                runSummaryMarkdownPath = staleMarkdownPath
            },
        ...
        var exitCode = await app.RunAsync(["replay", "packet", "--check", "--artifacts", staleRoot]);
        using var envelope = console.ParseSingleOutputAsJson();

        Assert.Equal(2, exitCode);
        Assert.False(envelope.RootElement.GetProperty("ok").GetBoolean());
        Assert.Equal("usage_error", envelope.RootElement.GetProperty("error").GetProperty("category").GetString());
        Assert.Contains("points at artifact root", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
        Assert.Contains("Re-run `luotsi replay packet", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
    }

    [Fact]
    public async Task RunAsync_ReplayPacket_Check_MissingIndexEntrypoint_ReturnsUsageError()
    {
        var console = new FakeConsole();
        var fileSystem = new FakeFileSystem();
        var replayRoot = SeedReplaySummaryArtifacts(fileSystem);
        ...
        var writeExitCode = await app.RunAsync(["replay", "packet", "--artifacts", replayRoot]);
        fileSystem.DeleteFile(Path.Join(replayRoot, "index.html"));
        console.OutputLines.Clear();
        var checkExitCode = await app.RunAsync(["replay", "packet", "--check", "--artifacts", replayRoot]);
        using var envelope = console.ParseSingleOutputAsJson();
        ...
        Assert.False(envelope.RootElement.GetProperty("ok").GetBoolean());
        Assert.Equal("usage_error", envelope.RootElement.GetProperty("error").GetProperty("category").GetString());
        Assert.Contains("missing index HTML", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
        Assert.Contains("Re-run `luotsi replay packet", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
    }
    ...
    public async Task RunAsync_ReplayPacket_Check_CopyPasteBlockWithoutChecklistCommand_ReturnsUsageError()
    {
        ...
        1. Run `luotsi artifacts open /tmp/replay-root`
        """);
        console.OutputLines.Clear();
        var checkExitCode = await app.RunAsync(["replay", "packet", "--check", "--artifacts", replayRoot]);
        using var envelope = console.ParseSingleOutputAsJson();
    ...
    [Fact]
    public async Task RunAsync_ReplayPacket_Check_CopyPasteBlockWithoutGateFirst_ReturnsUsageError()
    {
        var console = new FakeConsole();
        var fileSystem = new FakeFileSystem();
        var replayRoot = SeedReplaySummaryArtifacts(fileSystem);
        ...
        Assert.False(envelope.RootElement.GetProperty("ok").GetBoolean());
        Assert.Equal("usage_error", envelope.RootElement.GetProperty("error").GetProperty("category").GetString());
        Assert.Contains("copy-paste triage command block must start with the packet validation gate command", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
        Assert.Contains("Re-run `luotsi replay packet", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
    }
    ...
    [Fact]
    public async Task RunAsync_ReplayPacket_Check_MarkdownChecklistWithoutStructuredActions_ReturnsUsageError()
    {
        var console = new FakeConsole();
        var fileSystem = new FakeFileSystem();
        var replayRoot = SeedReplaySummaryArtifacts(fileSystem);
        ...
        1. Open the artifact browser only.
        """);
        console.OutputLines.Clear();
        var checkExitCode = await app.RunAsync(["replay", "packet", "--check", "--artifacts", replayRoot]);
        using var envelope = console.ParseSingleOutputAsJson();
    ...
    [Fact]
    public async Task RunAsync_ReplayPacket_Check_MarkdownWithoutPrimaryFailureDetails_ReturnsUsageError()
    {
        var console = new FakeConsole();
        var fileSystem = new FakeFileSystem();
        var replayRoot = SeedReplaySummaryArtifacts(fileSystem);
        ...
        var writeExitCode = await app.RunAsync(["replay", "packet", "--artifacts", replayRoot]);
        var packet = JsonNode.Parse(await fileSystem.ReadAllTextAsync(Path.Join(replayRoot, "run-summary.json")))!.AsObject();
        var recommendedAction = packet["recommendedNextAction"]!.AsObject();
        var recommendedCommand = recommendedAction["command"]!.GetValue<string>();
        var recommendedTitle = recommendedAction["title"]!.GetValue<string>();
        var recommendedKind = recommendedAction["kind"]!.GetValue<string>();
        var recommendedReason = recommendedAction["reason"]!.GetValue<string>();
        var primarySourceCommand = packet["primaryFailure"]!.AsObject()["sourceCommand"]!.GetValue<string>();
        fileSystem.AddFile(Path.Join(replayRoot, "run-summary.md"), $"""
        # Luotsi Run Summary
        ...
        - **{recommendedTitle}** (`{recommendedKind}`)
          {recommendedReason}
          `{recommendedCommand}`
        """);
        console.OutputLines.Clear();
        ...
        Assert.False(envelope.RootElement.GetProperty("ok").GetBoolean());
        Assert.Equal("usage_error", envelope.RootElement.GetProperty("error").GetProperty("category").GetString());
        Assert.Contains("primary failure detail section", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
        Assert.Contains("Re-run `luotsi replay packet", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
    }
    ...
    [Fact]
    public async Task RunAsync_ReplayPacket_Check_MissingVerdict_ReturnsUsageError()
    {
        var console = new FakeConsole();
        var fileSystem = new FakeFileSystem();
        var replayRoot = SeedReplaySummaryArtifacts(fileSystem);
        ...
        Assert.False(envelope.RootElement.GetProperty("ok").GetBoolean());
        Assert.Equal("usage_error", envelope.RootElement.GetProperty("error").GetProperty("category").GetString());
        Assert.Contains("verdict", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
    }

    [Fact]
    public async Task RunAsync_ReplayPacket_Check_MissingGeneratedAt_ReturnsUsageError()
    {
        var console = new FakeConsole();
        var fileSystem = new FakeFileSystem();
        var replayRoot = SeedReplaySummaryArtifacts(fileSystem);
        ...
        Assert.False(envelope.RootElement.GetProperty("ok").GetBoolean());
        Assert.Equal("usage_error", envelope.RootElement.GetProperty("error").GetProperty("category").GetString());
        Assert.Contains("generatedAt", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
    }

    [Fact]
    public async Task RunAsync_ReplayPacket_Check_InvalidGeneratedAt_ReturnsUsageError()
    {
        var console = new FakeConsole();
        var fileSystem = new FakeFileSystem();
        var replayRoot = SeedReplaySummaryArtifacts(fileSystem);
        ...
        var packetPath = Path.Join(replayRoot, "run-summary.json");
        var packet = JsonNode.Parse(await fileSystem.ReadAllTextAsync(packetPath))!.AsObject();
        packet["generatedAt"] = "not-a-timestamp";
        fileSystem.AddFile(packetPath, packet.ToJsonString());
        console.OutputLines.Clear();
        var checkExitCode = await app.RunAsync(["replay", "packet", "--check", "--artifacts", replayRoot]);
        using var envelope = console.ParseSingleOutputAsJson();
        ...
        Assert.Equal("usage_error", envelope.RootElement.GetProperty("error").GetProperty("category").GetString());
        Assert.Contains("generatedAt", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
        Assert.Contains("RFC 3339 timestamp", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
    }
    ...
    public async Task RunAsync_ReplayScenarioDraft_Writes_Valid_Draft_From_Inspect_Timeline()
    {
        ...
        Assert.Equal($"luotsi replay packet --artifacts {replayRoot}", suggestedCommands[0].GetProperty("command").GetString());
        Assert.Equal($"luotsi replay packet --artifacts {replayRoot} --check", suggestedCommands[1].GetProperty("command").GetString());
    ...
    public async Task RunAsync_ReplayGraph_Returns_Semantic_Debug_Seed_Graph()
    {
        ...
        Assert.Equal("write_replay_packet", data.GetProperty("actions")[0].GetProperty("kind").GetString());
        Assert.Equal($"luotsi replay packet --artifacts {replayRoot}", data.GetProperty("actions")[0].GetProperty("command").GetString());
        Assert.Equal("check_replay_packet", data.GetProperty("actions")[1].GetProperty("kind").GetString());
        Assert.Equal($"luotsi replay packet --artifacts {replayRoot} --check", data.GetProperty("actions")[1].GetProperty("command").GetString());
    ...
    [Fact]
    public async Task RunAsync_ArtifactsInfo_PackageZip_Returns_Manifest_Redaction_And_Intake_Commands()
    {
        var fileSystem = new FakeFileSystem();
        var console = new FakeConsole();
        var replayRoot = "/tmp/luotsi/20260526-120000-run";
        var packagePath = "/tmp/share/replay.zip";
        var defaultOutputDirectory = Path.GetFullPath("/tmp/share/replay");
        fileSystem.CreateDirectory(replayRoot);
        ...
        var exitCode = await app.RunAsync(["artifacts", "info", packagePath]);
        using var envelope = console.ParseSingleOutputAsJson();
        ...
        Assert.Equal(defaultOutputDirectory, data.GetProperty("default_output_directory").GetString());
        var sha256 = data.GetProperty("sha256").GetString();
        Assert.Matches("^[0-9a-f]{64}$", sha256);

        var manifest = data.GetProperty("manifest");
        Assert.Equal("luotsi-artifact-package.v1", manifest.GetProperty("schema").GetString());
        Assert.Equal(2, manifest.GetProperty("source_file_count").GetInt32());
        Assert.Equal("lab-safe", manifest.GetProperty("redaction").GetProperty("mode").GetString());
    ...
    [Fact]
    public async Task RunAsync_ArtifactsInfo_PackageZip_Rejects_Invalid_Package_Without_Writing()
    {
        var fileSystem = new FakeFileSystem();
        var console = new FakeConsole();
        var packagePath = "/tmp/share/bad.zip";
        var defaultOutputDirectory = Path.GetFullPath("/tmp/share/bad");
        ...
        await using (var packageStream = fileSystem.OpenWrite(packagePath))
        {
            ...
            await using (var writer = new StreamWriter(entry))
            {
                ...
                {"schema":"luotsi-artifact-package.v1","run_id":"20260526-120000-run","created_at":"2026-05-26T12:00:00Z","source_file_count":1,"category_counts":{"screenshots":0,"videos":0,"reports":0,"logs":0,"timelines":0,"other":1},"recommended_commands":[],"files":["index.html"]}
                """);
            }
        }
        ...
        Assert.False(fileSystem.DirectoryExists(defaultOutputDirectory));
        Assert.False(fileSystem.FileExists(Path.Join(defaultOutputDirectory, "index.html")));
    }

    [Fact]
    public async Task RunAsync_ArtifactsInfo_PackageZip_Recommends_Force_When_Default_Output_Exists()
    {
        var fileSystem = new FakeFileSystem();
        var console = new FakeConsole();
        var replayRoot = "/tmp/luotsi/20260526-120000-run";
        ...
        fileSystem.AddFile(Path.Join(defaultOutputDirectory, "index.html"), "existing");
        console.OutputLines.Clear();
        console.ErrorLines.Clear();
        ...
        Assert.Contains(data.GetProperty("recommended_commands").EnumerateArray(), command =>
            command.GetProperty("kind").GetString() == "unpack_artifacts" &&
            command.GetProperty("command").GetString() == $"luotsi artifacts unpack {packagePath} --output {defaultOutputDirectory} --force --sha256 {sha256}");
        Assert.Contains(data.GetProperty("recommended_commands").EnumerateArray(), command =>
            command.GetProperty("kind").GetString() == "unpack_artifacts_dry_run" &&
            command.GetProperty("command").GetString() == $"luotsi artifacts unpack {packagePath} --output {defaultOutputDirectory} --force --dry-run --sha256 {sha256}");
        Assert.Equal("existing", await fileSystem.ReadAllTextAsync(Path.Join(defaultOutputDirectory, "index.html")));
    }

    [Fact]
    public async Task RunAsync_ArtifactsInfo_PackageZip_Rejects_Missing_Package()
    {
        var console = new FakeConsole();
        var app = new App(new AppDependencies { Console = console });

        var exitCode = await app.RunAsync(["artifacts", "info", "/tmp/share/missing.zip"]);
        using var envelope = console.ParseSingleOutputAsJson();

        Assert.Equal(2, exitCode);
        Assert.Equal("usage_error", envelope.RootElement.GetProperty("error").GetProperty("category").GetString());
        Assert.Contains("Artifact package '/tmp/share/missing.zip' does not exist", envelope.RootElement.GetProperty("error").GetProperty("message").GetString(), StringComparison.Ordinal);
    }

    [Fact]
    public async Task RunAsync_ArtifactsInfo_DirectoryEndingZip_Remains_ArtifactRoot()
    {
        var fileSystem = new FakeFileSystem();
        var console = new FakeConsole();
        var replayRoot = "/tmp/artifacts/20260526-120000-run.zip";
        fileSystem.CreateDirectory(replayRoot);
        fileSystem.AddFile(Path.Join(replayRoot, "session-timeline.jsonl"), "{\"type\":\"session_started\"}");
        var app = new App(new AppDependencies
        {
        ...
        var data = envelope.RootElement.GetProperty("data");
        Assert.Equal(replayRoot, data.GetProperty("artifact_root").GetString());
        Assert.Equal("20260526-120000-run.zip", data.GetProperty("run_id").GetString());
        Assert.True(data.GetProperty("has_timeline").GetBoolean());
    }

    [Fact]
    public async Task RunAsync_ArtifactsInfo_RunIdEndingZip_Resolves_From_Search_Root()
    {
        var fileSystem = new FakeFileSystem();
        var console = new FakeConsole();
        var searchRoot = "/tmp/artifacts";
        var replayRoot = Path.Join(searchRoot, "20260526-120000-run.zip");
        fileSystem.CreateDirectory(searchRoot);
        fileSystem.AddFile(Path.Join(replayRoot, "session-timeline.jsonl"), "{\"type\":\"session_started\"}");
        var app = new App(new AppDependencies
        {
        ...
        var exitCode = await app.RunAsync(["artifacts", "info", "20260526-120000-run.zip", "--artifacts", searchRoot]);
        using var envelope = console.ParseSingleOutputAsJson();
    ...
    public async Task RunAsync_ArtifactsPack_Writes_Zip_With_Relative_Entries()
    {
        ...
        var sha256 = data.GetProperty("sha256").GetString();
        Assert.Matches("^[0-9a-f]{64}$", sha256);
        var defaultUnpackRoot = Path.Join(Path.GetDirectoryName(Path.GetFullPath(output)), Path.GetFileNameWithoutExtension(output));
        Assert.Contains(data.GetProperty("recommended_commands").EnumerateArray(), command =>
            command.GetProperty("kind").GetString() == "unpack_artifacts" &&
    ...
    [Fact]
    public async Task RunAsync_ArtifactsUnpack_Sha256Match_Verifies_Package()
    {
        var fileSystem = new FakeFileSystem();
        var console = new FakeConsole();
        var replayRoot = "/tmp/luotsi/20260526-120000-run";
        ...
        var exitCode = await app.RunAsync(["artifacts", "unpack", packagePath, "--output", "/tmp/unpacked", "--sha256", sha256!]);
        using var envelope = console.ParseSingleOutputAsJson();
    ...
    [Fact]
    public async Task RunAsync_ArtifactsUnpack_Sha256Mismatch_Fails_Before_Writing()
    {
        var fileSystem = new FakeFileSystem();
        var console = new FakeConsole();
        var replayRoot = "/tmp/luotsi/20260526-120000-run";
        ...
        var exitCode = await app.RunAsync(["artifacts", "unpack", packagePath, "--output", "/tmp/unpacked", "--sha256", wrongSha256]);
        using var envelope = console.ParseSingleOutputAsJson();
    ...
    [Fact]
    public async Task RunAsync_ArtifactsUnpack_Sha256Rejects_Invalid_Digest()
    {
        var fileSystem = new FakeFileSystem();
        var console = new FakeConsole();
        var replayRoot = "/tmp/luotsi/20260526-120000-run";
        ...
        var exitCode = await app.RunAsync(["artifacts", "unpack", packagePath, "--output", "/tmp/unpacked", "--sha256", "not-a-digest"]);
        using var envelope = console.ParseSingleOutputAsJson();
